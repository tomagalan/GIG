...
monObj = bpy.context.scene.objects['gargoyle']
...
for face in monObj.data.polygons:
    ...
    for neigh in FA[face.index]:
        angle_avg += angle_measure(face, monObj.data.polygons[neigh])
    angle_avg /= 3
    if math.fabs(angle_avg) < white_limit:
        setColor(monObj, face.index, "blanc")
    elif angle_avg < 0:
        if math.fabs(angle_avg) > extreme_limit:
